"""
Chaper 2 code sucks! It has froze my computer twice now.

Chapter 3 uses a library that is not compatible with python 3.x

Chapter 4 hopefully is better.
"""
    ...
    def draw_board(self):
        ...
        for r in range(self.rows):
            ...
            for c in range(self.columns):
                ...
                y1 = y1 + self.dim_square
                self.canvas.create_rectangle(x1, y1, x2, y2, fill=color, tag="area")
                color = self.color1 if color == self.color2 else.color2
